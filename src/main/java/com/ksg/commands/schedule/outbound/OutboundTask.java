...
public class OutboundTask extends SimpleTask{
	...
	private ScheduleBuildMessageDialog processMessageDialog;

	public OutboundTask() {
		super();
		logger.info("outbound 스케줄 생성 시작");
		try 
		{
			initTag();
			makeSchedule();
		...
		} finally
		{
			...
			if(this.processMessageDialog!=null){
				this.processMessageDialog.setVisible(false);
				this.processMessageDialog.dispose();
			}
		}

	}

	/*	private void initTag() throws SQLException {
		logger.info("태그정보 초기화");
		Code code_info = new Code();
		code_info.setCode_name("BOLD_TAG_B");

		Code b=baseService.getCodeInfo(code_info);

		BOLD_TAG_B="<ct:>";

		code_info.setCode_name("BOLD_TAG_F");
		Code f=baseService.getCodeInfo(code_info);

//*****		BOLD_TAG_F="<ct:Narrow Bold>";
//*****2		BOLD_TAG_F="<ct:77 Bold Condensed>";
		BOLD_TAG_F="<ct:Bold Condensed>";

		code_info.setCode_name("INBOUND_VERSION");
		Code f0=baseService.getCodeInfo(code_info);
		TAG_VERSION0="<KSC5601-WIN>";


		code_info.setCode_name("INBOUND_VERSION2");
		Code f2=baseService.getCodeInfo(code_info);
		TAG_VERSION2="<vsn:8><fset:InDesign-Roman><ctable:=<검정:COLOR:CMYK:Process:0,0,0,1>>";

		code_info.setCode_name("INBOUND_VERSION3");
...
//*****		TAG_VERSION3="<dps:정규=<Nextstyle:정규><cc:검정><cs:8.000000><clig:0><cbs:-0.000000><phll:0><pli:53.858291><pfli:-53.858292><palp:1.199996><clang:Neutral><ph:0><pmcbh:3><phc:0><pswh:6><phz:0.000000><ptr:19.842498779296875\\,Left\\,.\\,0\\,\\;211.3332977294922\\,Right\\,.\\,0\\,\\;><cf:Helvetica LT Std><pmaws:1.500000><pmiws:1.000000><pmaxl:0.149993><prac:검정><prat:100.000000><prbc:검정><prbt:100.000000><blf:\\<TextFont\\>><bltf:\\<TextStyle\\>>>";
		...
		code_info.setCode_name("INBOUND_VERSION4");
		...
		code_info.setCode_name("INBOUND_VERSION5");
		Code f5=baseService.getCodeInfo(code_info);
//*****2		TAG_VERSION5="<pstyle:Body Text><ptr:19.842498779296875\\,Left\\,.\\,0\\,\\;211.3332977294922\\,Right\\,.\\,0\\,\\;><cs:8.000000><cl:5.479995>\r\n";
	...
	{	
		logger.debug("port from port arrange");
	...
	{	
		...
		for(int i=0;i<fromPort.length;i++)
		{
			...
			{
				tempFromPort.remove(fromPort[i]);
				logger.debug("remove port : "+fromPort[i]);
			}
		}
		return tempFromPort;
	}
	...
	IOException {
		try {
			...
			processMessageDialog.setMessage("create outbound xml file...");
	...
	private void makeXTG2(Document document) throws IOException {
		...
		processMessageDialog.setMessage("create outbound xtg file...");
		Element root = document.getRootElement();
				...
				for (int vesselcount = 0; vesselcount < vessellist.size(); vesselcount++) 
				{
					...
					try {
						...
						{	
							...
							Vessel info = baseService.getVesselInfo(op);
	...
	public void makeSchedule() throws SQLException, JDOMException, IOException {
		...
		processMessageDialog.setMessage("outbound schedule making...");
