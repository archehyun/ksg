...
import com.ksg.schedule.build.SortUtil;
 ...
 *
 */
public class OutboundScheduleBuildV2 extends DefaultScheduleBuild{
	...
	public OutboundScheduleBuildV2() throws SQLException {
	...
	class ToPortGroup extends HashMap<String,FromPortGroup>
	{
		...
		public void addSchedule(ScheduleData data) throws SQLException, VesselNullException {
			...
			if(info.getPort_name().equals(DefaultScheduleBuild.BUSAN_NEW_PORT))
			{
				data.setFromPort(DefaultScheduleBuild.BUSAN);
			}				
	...
	class VesselGroup extends  ArrayList<ScheduleData> implements Comparable<Object>
	{
		...
		private String toShippingString(String dateF, String vessel, String company, String agent, String dateT)
		{
			try {
				...
				return buildVesselXTG(formatedDateF, vessel, formatedCompany, vessel_type, formatedDateT);
	...
	class CompanyString implements Comparable<CompanyString>
	{
		...
		public String toString()
		{
			return companyName.equals(agent)?companyName:companyName+"/"+agent;
		}
			...
			while(toPortIter.hasNext())
			{
				try{
					...
					for(int i=0;i<fromPortArray.length;i++)
					{
						...
						fw.write(buildFromXTG(i, fromPortGroup.fromPortName));
