 ...
 *        
 */
public class RouteScheduleBuild extends DefaultScheduleBuild{
	...
	public RouteScheduleBuild(ShippersTable op) throws SQLException {
		super();
		this.op = op;
		initTag();
	}

	public RouteScheduleBuild(ShippersTable op, int orderBy) throws SQLException {
		
		this(op);
		
		this.orderByType =orderBy;
		
	//	li =scheduleService.getOutboundScheduleList();
...
/*			while(iter.hasNext())
	...
	/*		GroupArea[] areaGroups=scheduleGroup.toSortedArray();
			...
			message = "스케줄 출력..";
			logger.info("스케줄 출력 시작");
			lengthOfTask =areaGroups.length;
			current=0;
